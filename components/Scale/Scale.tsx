...
const Scale = () => {
  ...
  const { isLoading, error, data, refetch } = useQuery(
    ...
    () => GET_PRODUCTS_BY_DAY(),
    {
      onSuccess: ({data}) => setCurrentSum(data.reduce((acc: number, item: TProduct) => acc += item.price, 0)),
    }
  );
  ...
  useEffect(() => {
    ...
    if(currentSpending >= 100) {
      ...
      circleRef.current.style.left = `${currentSpending - 2}%`;
    }
  ...
  return (
    <div className="my-6 ">
      <div className="h-[6px] border-y border-y-[black] mb-4 relative">
        <div
          ref={lineRef}
          className={`h-[6px] bg-green border-b border-b-[black] mb-4 absolute transition-all ease-in duration-[1s]`}></div>
        <div
          ref={circleRef}
          className={`h-[13px] w-[13px] border border-[black] rounded-full bg-green absolute translate-y-[50%] bottom-[50%] transition-all ease-in duration-[1s]`}></div>
        <div
          className={`py-2 px-3 border border-[black] rounded-full bg-yellow absolute translate-y-[50%] bottom-[50%] transition-all right-0 `}>{userInfo.dayLimit}</div>
      </div>
